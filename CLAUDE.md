...
- **Two-pass AI** to handle 20–50k song libraries without exceeding context limits:
  ...
  - SQLite query narrows to up to 500 candidates, biased by popularity with random jitter. If fewer than 500 match but enough exist to fill the playlist, they pass directly to Pass 2 without broadening — keeps results focused for artist-specific or niche queries. Broadening (genre-only, then unfiltered) only triggers when candidates < requested song count.
...
- **Mood/theme tagging**: Essentia MTG-Jamendo model classifies tracks into a standardized vocabulary of 31 mood tags (happy, sad, energetic, calm, etc.) and 26 theme tags (film, party, nature, summer, etc.) with confidence scores. Essentia is the sole source for mood/theme data to ensure consistent, standardized tags across the library. Tags are stored with confidence scores (e.g. `happy:0.85, energetic:0.72`) in separate `mood_tags` and `theme_tags` columns. The full vocabulary is provided to the AI in Pass 1 so it can map natural language prompts to valid filter tags. The raw file metadata `mood` field remains in the DB but is not used for filtering or passed to the AI. Configurable: enable/disable, batch size (X tracks), interval (Y hours after batch completes). Models auto-download from `essentia.upf.edu` on first use (~80MB). Requires `essentia-tensorflow`.
